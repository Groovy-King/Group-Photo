function [idx_new,p,MASS_EST] = Find_PMlG_after_filter_viral_AllT(vol,slice,idx,~)
                                ...
                                for pos=1:length(idx)
                                   ...
                                   for all_cone_vol=1: 9
                                      for all_cone_slice=1:3
                                           ...
                                           [~,IX]=sort(Lum,'descend');
                                           Xdata=Xdata(IX,:);
                                          load(strcat('Expectedpr3_vir_mass_no_5_',num2str(all_cone_vol),'_',num2str(all_cone_slice),'.mat'));
                                          clear MASS_EST mass_no_final_md mass_no_final_mn New_idx P_GlM P_MlG;
                                          EXP_GRP=Xgrid;
                                            %%Filter idx_rf if it is group
                                            %%or likely to be will remain
                                            %%otherwise remove it from the
                                            %%list
                                            idx_rf=find(Xdata(:,3)>(XdataBase(idx(pos),3)-(tolar/1000)) & Xdata(:,3)<(XdataBase(idx(pos),3)+(tolar/1000)));
                                            
                                            
                                            
                                            if ~isempty( idx_rf)
                                                                    ididid=find(ismember(Xdata(idx_rf,:),EXP_GRP,'rows'));
                                          ...
                                          %%check the gaussainity
                                            if ~isempty( idx_rf)
                                                                ...
                                                                for curr_ms=1:length(indexx)
                                                                                ...
                                                                                load(strcat('Set_mass_vol',num2str(all_cone_vol),'_slice_',num2str(all_cone_slice),'_',num2str(mass),'.mat'));
                                                                                Set_b=Set(idx_rf);
                                                                            ...
                                                                            %%%%%%%%%%%
                                                                            if ~isempty(Set_b(Flagidx))
                                                                                if isempty(Set_b_mas{curr_ms})
                                                                                    if length(Flagidx)==1
                                                                                        Set_b_mas(curr_ms)=Set_b(Flagidx);
                                                                                    else
                                                                                        for kl=1:length(Flagidx)
                                                                                            Set_b_mas(curr_ms,length(Set_b_mas(curr_ms,:))+1)=Set_b(Flagidx(kl));
                                                                                        end
                                                                                    end
                                                                                else
                                                                                    if length(Flagidx)==1
                                                                                        Set_b_mas(curr_ms,length(Set_b_mas(curr_ms,:))+1)=Set_b(Flagidx);
                                                                                    else
                                                                                        for kl=1:length(Flagidx)
                                                                                            Set_b_mas(curr_ms,length(Set_b_mas(curr_ms,:))+1)=Set_b(Flagidx(kl));
                                                                                        end
                                                                ...
                                                                for curr_ms=1:length(indexx)
                                                                                ...
                                                                                Set_b=Set_b_mas(curr_ms,~cellfun(@isempty,Set_b_mas(curr_ms,:)));
                                            ...
                                            %% Find the corresponding index of the closest count from (xcentr) then get the corresponding value from the (P_M_l_z_c)
                                                        ...
                                                        for msms=1:10
                                                            mass=idxidx(msms);
                                                            sdt=load(strcat('Set_mass_vol',num2str(all_cone_vol),'_slice_',num2str(all_cone_slice),'_',num2str(mass),'.mat'));
